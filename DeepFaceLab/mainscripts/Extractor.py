                    ...
                    if self.cache_text_lines_img[0] == sh:
                        ...
                        self.text_lines_img = (imagelib.get_draw_text_lines ( self.image, sh,
                                                        [   '[L Mouse click] - lock/unlock selection. [Mouse wheel] - change rect',
                                                            ...
                                                            '[u] [j] - skip forward/backward 10 frames',
                                                            '[i] [k] - skip forward/backward 50 frames',
                                                            '[o] [l] - skip forward/backward 100 frames',
                                                            '[p] [;] - skip forward/backward 500 frames',
                ...
                if len(data_rects) == 0:
                    ...
                    while True:
                        ...
                        if key == ord('\r') or key == ord('\n'):
                            ...
                            interval = min(10,len(self.input_data))
                            for i in range(interval):
                                self.result.append( self.input_data.pop(0) )
                            io.progress_bar_inc(interval)
                            break
                        elif key == ord('j'):
                            interval = min(10,len(self.result))
                            for i in range(interval):
                                self.input_data.insert(0, self.result.pop() )
                            io.progress_bar_inc(-interval)
                            break
                        elif key == ord('i'):
                            interval = min(50,len(self.input_data))
                            for i in range(interval):
                                self.result.append( self.input_data.pop(0) )
                            io.progress_bar_inc(interval)
                            break
                        elif key == ord('k'):
                            interval = min(50,len(self.result))
                            for i in range(interval):
                                self.input_data.insert(0, self.result.pop() )
                            io.progress_bar_inc(-interval)
                            break
                        elif key == ord('o'):
                            interval = min(100,len(self.input_data))
                            for i in range(interval):
                                self.result.append( self.input_data.pop(0) )
                            io.progress_bar_inc(interval)
                            break
                        elif key == ord('l'):
                            interval = min(100,len(self.result))
                            for i in range(interval):
                                self.input_data.insert(0, self.result.pop() )
                            io.progress_bar_inc(-interval)
                            break
                        elif key == ord('p'):
                            interval = min(500,len(self.input_data))
                            for i in range(interval):
                                self.result.append( self.input_data.pop(0) )
                            io.progress_bar_inc(interval)
                            break
                        elif key == ord(';'):
                            interval = min(500,len(self.result))
                            for i in range(500):
                                self.input_data.insert(0, self.result.pop() )
                            io.progress_bar_inc(-interval)
                            break
